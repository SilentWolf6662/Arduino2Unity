...
public class SerialController : MonoBehaviour
{
    ...
    [Tooltip("Reference to an scene object that will receive the events of connection, disconnection and the messages from the serial device.")]
    ...
    private void Update()
    {
        ...
        if (ReferenceEquals(message, SERIAL_DEVICE_CONNECTED)) messageListener.SendMessage("OnConnectionEvent", true);
        else if (ReferenceEquals(message, SERIAL_DEVICE_DISCONNECTED)) messageListener.SendMessage("OnConnectionEvent", false);
        else messageListener.SendMessage("OnMessageArrived", $"{message}");
    }
    ...
    // ------------------------------------------------------------------------
    // Read the next message from the queue
    public string ReadSerialMessage() => (string)serialThread.ReadMessage();

    // ------------------------------------------------------------------------
    // Puts a message in the outgoing queue. The thread object will send the
    // message to the serial device when it considers it's appropriate.
    // ------------------------------------------------------------------------
    public void SendSerialMessage(string message) => serialThread.SendMessage(message);
    ...
    public void SetTearDownFunction(TearDownFunction userFunction) => userDefinedTearDownFunction = userFunction;
