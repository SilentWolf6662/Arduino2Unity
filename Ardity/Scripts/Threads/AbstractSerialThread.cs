...
public abstract class AbstractSerialThread
{
    ...
    private readonly int delayBeforeReconnecting;
    private readonly int maxUnreadMessages;
    ...
    private readonly bool enqueueStatusMessages;
    ...
    // ------------------------------------------------------------------------
    protected AbstractSerialThread(string portName, int baudRate, int delayBeforeReconnecting, int maxUnreadMessages, bool enqueueStatusMessages)
    {
    ...
    // ------------------------------------------------------------------------
    public object ReadMessage() => inputQueue.Count == 0 ? null : inputQueue.Dequeue();
    ...
    // ------------------------------------------------------------------------
    public void SendMessage(object message) => outputQueue.Enqueue(message);
        ...
        try
        {
            while (!IsStopRequested())
                ...
                catch (Exception ioe)
                {
                    ...
                    if (enqueueStatusMessages) inputQueue.Enqueue(SerialController.SERIAL_DEVICE_DISCONNECTED);
            ...
            while (outputQueue.Count != 0) SendToWire(outputQueue.Dequeue(), serialPort);
    ...
    private void AttemptConnection()
    {
        ...
        if (enqueueStatusMessages) inputQueue.Enqueue(SerialController.SERIAL_DEVICE_CONNECTED);
    }
    ...
    {
        lock (this) return stopRequested;
    }
    ...
    private void RunOnce()
    {
        ...
        {
            // Send a message.
            if (outputQueue.Count != 0) SendToWire(outputQueue.Dequeue(), serialPort);
            ...
            {
                if (inputQueue.Count < maxUnreadMessages) inputQueue.Enqueue(inputMessage);
                else Debug.LogWarning($"Queue is full. Dropping message: {inputMessage}");
            }
        }
